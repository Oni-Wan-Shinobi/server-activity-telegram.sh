...
while IFS= read -r line; do
    ...
    if echo "$line" | grep -q "Accepted"; then
        ...
        IP=$(echo "$line" | grep -oP 'from \K[0-9a-f.:]+' || echo "unknown")
...
while true; do
    while IFS= read -r line; do
        ...
        # ── SYSCALL: запоминаем auid + pid для следующего EXECVE ──────────────
        if [[ "$line" == *"type=SYSCALL"* ]]; then
            ...
            # Пропускаем если auid не прошёл (на всякий случай, auditd уже фильтрует)
        ...
        # ── EXECVE: собираем команду ──────────────────────────────────────────
        if [[ "$line" == *"type=EXECVE"* ]]; then
            eid=$(echo "$line" | grep -oP 'msg=audit\([0-9]+:\K[0-9]+' || echo "")

            # EXECVE без парного SYSCALL — пропускаем
            [ "$eid" != "$pending_eid" ] || [ -z "$pending_eid" ] && continue

            auid="$pending_auid"
            pid="$pending_pid"
            pending_eid=""   # сбрасываем — каждая пара обрабатывается один раз
            ...
            # Собираем аргументы команды из a0, a1, a2...
            ...
            NOW=$(date '+%Y-%m-%d %H:%M:%S')
            ts_raw=$(echo "$line" | grep -oP 'msg=audit\(\K[0-9]+' || echo "")

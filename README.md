...
```bash
curl -fsSL https://raw.githubusercontent.com/Oni-Wan-Shinobi/server-activity-telegram.sh/main/server-activity-telegram.sh | sudo bash
```

Или если хотите просмотреть скрипт перед запуском:
...
Or if you want to inspect the script before running it:
